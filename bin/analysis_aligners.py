...
for index00, file00 in enumerate(simulated_files):
 ...
 print 'Read aligner file:', file2

 samfile_aligner = pysam.Samfile(file2, 'rb')
 for align in samfile_aligner.fetch():
    cnt+=1
    if (cnt % LOG_EVERY_N) == 0:
          print cnt
    readstart.append(int(align.pos) + 1)
    align_length = align.rlen
    readlength.append(align.rlen)
    read_end = int(align.pos) + align_length
    readend.append(read_end)
    readid.append(align.qname)
    strand = -1 if align.is_reverse else 1
    readstrand.append(strand)
    readseq.append(align.query)
    readcig.append(align.cigar)
    readFlag.append(int(align.flag))   
    try:
      readAS.append(int(align.opt('AS')))  
    except KeyError:
      readAS.append(None)    
    try:
      readMD.append(align.opt('MD'))
    except KeyError:
      readMD.append(None)    
 samfile_aligner.close()
 ...
 for read in readTrueId:
    ...
    if indx_aln != '':
        if readFlag[indx_aln] != 4:
            mapped_read.append(True)
            # is read crossing a junction?
            ...
            cig_aln = array(parse_cigar_MD_tags(readcig[indx_aln],readMD[indx_aln])) 
            cig_true = array(readTrueCig[indx])
            if readstrand[indx_aln] == readTrueStrand[indx]: 
                stats=base_accuracy(cig_aln,cig_true,readstart[indx_aln],readend[indx_aln],coordinate_change)
